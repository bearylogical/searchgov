...
{#if !$isAuthenticated}
	...
	<div class="flex-1 flex items-center justify-center p-8">
		<div class="text-center max-w-md">
			...
			<h1 class="text-2xl font-bold text-gray-900 mb-2">SGDI Analytics</h1>
			...
			{#each statCards as card}
				<div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
					<div class="flex items-start justify-between">
						<div>
							{#if statsLoading}
								<div class="h-8 w-24 bg-gray-100 rounded animate-pulse mb-1"></div>
						...
						<div
							class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0
								{card.color === 'blue' ? 'bg-blue-50' : card.color === 'violet' ? 'bg-violet-50' : 'bg-emerald-50'}"
						>
							<svg
								class="w-5 h-5
									{card.color === 'blue' ? 'text-blue-600' : card.color === 'violet' ? 'text-violet-600' : 'text-emerald-600'}"
		...
		<div>
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				{#each featureCards as card}
					<a
						href={card.href}
						class="group bg-white border border-gray-200 rounded-xl p-6 shadow-sm
					...
					>
						...
						<p class="text-sm text-gray-500 leading-relaxed mb-4">{card.description}</p>
						<span
							class="inline-flex items-center gap-1 text-sm font-medium text-blue-600
