...
<div class="flex-1 flex flex-col lg:flex-row overflow-hidden" style="height: calc(100vh - 3.5rem - 49px)">
	...
	<aside class="w-full lg:w-80 xl:w-96 shrink-0 flex flex-col border-b lg:border-b-0 lg:border-r border-gray-200 bg-white">
		...
		<div class="p-4 border-b border-gray-100">
			<h1 class="text-sm font-semibold text-gray-900 mb-3">Organisation Explorer</h1>
			<div class="relative">
				<svg
		...
		<div class="flex-1 overflow-y-auto">
			...
			{:else if !query}
				<div class="p-8 text-center">
					...
					<p class="text-sm text-gray-400">Search for an organisation to begin</p>
				</div>
			{:else}
				<ul>
					{#each results as org}
						<li>
							...
							>
								...
								<div class="min-w-0">
									...
									{#if org.department}
										<p class="text-xs text-gray-400 truncate">{org.department}</p>
									{/if}
								</div>
							</button>
						</li>
		...
		{#if !selected}
			<div class="h-full flex items-center justify-center p-8">
				<div class="text-center">
					...
					<p class="text-sm text-gray-400">Select an organisation to explore its hierarchy</p>
				</div>
			</div>
		{:else}
			<div class="max-w-2xl mx-auto p-6">
				...
				<div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm mb-6">
					<div class="flex items-start gap-4">
						...
						<div class="min-w-0 flex-1">
							...
							{#if selected.department}
								<p class="text-sm text-gray-500 mt-0.5">{selected.department}</p>
							{/if}
							{#if !loadingTree && tree.length > 0}
								<p class="text-xs text-gray-400 mt-1">{tree.length} sub-organisation{tree.length !== 1 ? 's' : ''}</p>
							{/if}
						</div>
					</div>
					...
					{#if timeline.length > 0}
						<div class="mt-4 pt-4 border-t border-gray-100 flex items-center gap-3 flex-wrap">
							<span class="text-xs font-medium text-gray-500">View at snapshot:</span>
				...
				{:else}
					<ul class="space-y-2">
						{#each tree as child}
							<li class="bg-white border border-gray-200 rounded-xl px-4 py-3.5 shadow-sm hover:shadow-md transition-shadow flex items-center gap-3">
								<div class="w-1 h-8 rounded-full bg-violet-200 shrink-0"></div>
								<div class="min-w-0">
