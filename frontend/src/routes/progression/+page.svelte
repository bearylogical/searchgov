					...
					{#each results as person}
						<li>
							<button
								...
								class="w-full text-left px-4 py-3 hover:bg-gray-50 dark:bg-gray-950 transition-colors
	...
	<section class="flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-950">
		...
		{:else}
			<div class="p-6 space-y-6">
				<!-- Profile header -->
				<div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-sm flex items-center gap-4">
					...
					{#if !loadingCareer && career.length > 0}
						<div class="ml-auto shrink-0 text-right">
							...
							<p class="text-xs text-gray-400 dark:text-gray-500">roles</p>
						</div>
					{/if}
				</div>
				...
				{:else if career.length === 0}
					<div class="text-center py-12">
						<p class="text-sm text-gray-400 dark:text-gray-500">No employment records found.</p>
					</div>
				{:else}
					<div class="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
						...
						<!-- ── Career Timeline ──────────────────── -->
						<div>
							<h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Career Timeline</h3>
							<div class="relative">
									...
									{#each career as entry}
										<li class="pl-7 relative">
											<div class="absolute left-0 top-3.5 w-[11px] h-[11px] rounded-full border-2 border-white ring-1
											            {isActive(entry) ? 'bg-blue-500 ring-blue-400' : 'bg-gray-300 ring-gray-200 dark:ring-gray-700'}">
											</div>
											<div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
												<div class="flex items-start justify-between gap-2 flex-wrap">
													...
													{#if isActive(entry)}
														<span class="inline-flex items-center gap-1 text-xs font-medium text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-full px-2 py-0.5 shrink-0">
						...
						<!-- ── Individual Profile ───────────────── -->
						<div class="space-y-4">
							<h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Individual Profile</h3>

							<!-- Summary card -->
							<div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm space-y-3">
								{#if aliases.length > 1}
									<div>
										<p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Also known as</p>
										<div class="flex flex-wrap gap-1.5">
											{#each aliases as alias}
												<span class="text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-full px-2.5 py-0.5">{alias}</span>
											{/each}
										</div>
									</div>
								{/if}
								...
								<div>
									<p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-0.5">Total roles</p>
