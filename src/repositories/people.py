...
class PeopleRepository(BaseRepository):
    def create(self, data: Dict[str, Any]) -> Optional[int]:
        ...
        with self.db.get_cursor() as cur:
            cur.execute(
                """
                INSERT INTO people (name, clean_name, tel, email, embedding, metadata)
                VALUES (%(name)s, %(clean_name)s, %(tel)s, %(email)s, %(embedding)s, %(metadata)s)
                ON CONFLICT (name) DO UPDATE SET
                    clean_name = COALESCE(EXCLUDED.clean_name, people.clean_name),
                    tel = COALESCE(EXCLUDED.tel, people.tel),
                    email = COALESCE(EXCLUDED.email, people.email),
                    embedding = COALESCE(EXCLUDED.embedding, people.embedding),
            ...
            """,
                {
                    "name": data["name"],
                    "clean_name": data.get("clean_name", data["name"]),
                    "tel": data.get("tel"),
