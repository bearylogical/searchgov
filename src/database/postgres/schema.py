...
class SchemaManager:
    ...
    async def _drop_tables(self, conn):
        ...
        await conn.execute(
            "DROP MATERIALIZED VIEW IF EXISTS colleague_pairs;"
        )
    ...
    async def _create_extensions(self, conn):
        ...
        for ext in extensions:
            ...
            self.logger.debug(f"Executed: {ext}")

        self.logger.info("Registered vector extension")
        await self.register_pgvector(conn)

    async def register_pgvector(self, conn):
    ...
    async def _create_tables(self, conn):
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS people (
                ...
                UNIQUE(name, disambiguation_key)
            );
        """)
        ...
        """)
        self.logger.info("Created organizations table with parent_org_id")
    ...
    async def refresh_materialized_views(self):
        ...
        async with self.db.acquire() as conn:
            await conn.execute(
                "REFRESH MATERIALIZED VIEW CONCURRENTLY colleague_pairs;"
            )
